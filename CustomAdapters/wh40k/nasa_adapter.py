"""
nasa_adapter.py  --  B-Roll Engine Custom Adapter  v2.0
========================================================
NASA public domain images via Openverse API.

Source:  Openverse source slug "nasa"
         ~124,000 public domain images from NASA Goddard, NASA HQ,
         NASA Johnson, Marshall Space Flight Center, Kennedy Space Center,
         and the Apollo Image Gallery.
...
Run:
    ...
    python nasa_adapter.py

Port: 3003  (40k=3000, artvee=3001, wikimedia=3002, nasa=3003, openverse=3005)
"""
...
def health():
    return jsonify({
        "status":      "ok",
        "name":        "NASA Adapter (Openverse)",
        "version":     "2.0",
        "description": (
            "~124,000 public domain NASA images via Openverse: space photography, "
            "Earth observation, aeronautics, and Apollo archive. "
...
def search():
    ...
    log.info("Searching NASA (Openverse) for %r (limit=%d)", q, limit)
...
if __name__ == "__main__":
    print("=" * 60)
    print("  NASA Adapter (Openverse)  v2.0")
    print(f"  http://localhost:{PORT}")
    print("=" * 60)
    print()
    print("  Searches NASA images via Openverse API.")
    print("  ~124,000 public domain space and aeronautics images.")
    print()
    ...
    print(f'    curl "http://localhost:{PORT}/search?q=saturn+rocket&limit=5"')
