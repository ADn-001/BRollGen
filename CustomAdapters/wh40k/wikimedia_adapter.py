"""
wikimedia_adapter.py  --  B-Roll Engine Custom Adapter  v1.0
=============================================================
Wikimedia Commons via Openverse API.

Source:  https://commons.wikimedia.org/
         60+ million freely licensed images: photographs, artworks, maps,
         diagrams, and digitized collections from hundreds of GLAMs worldwide.
         Includes Library of Congress uploads, British Library digitizations,
         Europeana contributions, and millions of community uploads.

Openverse source slug:  wikimedia
...
Run:
    ...
    python wikimedia_adapter.py

Port: 3002  (40k=3000, artvee=3001, wikimedia=3002, nasa=3003, openverse=3005)
"""
...
def health():
    return jsonify({
        "status":      "ok",
        "name":        "Wikimedia Commons Adapter (Openverse)",
        "version":     "1.0",
        "description": (
            "60+ million freely licensed images from Wikimedia Commons via Openverse. "
            "Includes photographs, digitized artworks, maps, and GLAM collections. "
...
def search():
    ...
    log.info("Searching Wikimedia Commons (Openverse) for %r (limit=%d)", q, limit)
    raw = ov.openverse_search(query=q, limit=limit, source=SOURCE)
    if raw is None:
...
if __name__ == "__main__":
    print("=" * 60)
    print("  Wikimedia Commons Adapter (Openverse)  v1.0")
    print(f"  http://localhost:{PORT}")
    print("=" * 60)
    print()
    print("  Searches Wikimedia Commons via Openverse API.")
    print("  60+ million freely licensed images.")
    print()
    ...
    print(f'    curl "http://localhost:{PORT}/search?q=ancient+egypt&limit=5"')
